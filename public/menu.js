...
(function () {
  ...
  class ContextualMenuManager {
    ...
    insertRowBelow() {
      ...
      // ✅ Sauvegarder dans DOM Storage
      this.saveToDOMStorage();
    }
...
<table class="min-w-full border border-gray-200 dark:border-gray-700 rounded-lg n8n-result-table" style="margin-bottom: 16px;">
  ...
  <tbody>`;
    ...
    /**
     * Sauvegarder la table courante dans DOM Storage
     * ✅ NOUVEAU : Migration IndexedDB → DOM Storage
     */
    saveToDOMStorage() {
      try {
        if (!this.targetTable) {
          console.warn('⚠️ [Menu] Pas de table cible pour sauvegarde DOM Storage');
          return;
        }

        if (!window.domStorageManager) {
          console.warn('⚠️ [Menu] DOM Storage Manager non disponible');
          return;
        }

        const keyword = this.targetTable.dataset.keyword;
        if (!keyword) {
          console.warn('⚠️ [Menu] Table sans data-keyword, sauvegarde ignorée');
          return;
        }
        ...
        // Sauvegarder
        const success = window.domStorageManager.saveTable(sessionId, keyword, this.targetTable);

        if (success) {
          console.log(`💾 [Menu] Table sauvegardée dans DOM Storage: ${keyword}`);
        } else {
          console.error(`❌ [Menu] Échec sauvegarde DOM Storage: ${keyword}`);
        }
      } catch (error) {
        console.error('❌ [Menu] Erreur sauvegarde DOM Storage:', error);
      }
    }
    ...
    detectCurrentSessionId() {
      ...
      // Fallback
      console.warn('⚠️ [Menu] SessionId non détecté, utilisation fallback');
