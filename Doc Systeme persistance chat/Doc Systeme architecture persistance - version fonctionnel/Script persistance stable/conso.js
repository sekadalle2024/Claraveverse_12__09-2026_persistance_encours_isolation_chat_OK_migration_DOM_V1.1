...
(function () {
  ...
  class ClaraverseTableProcessor {
    ...
    getCurrentSessionId() {
      // 1. Via sessionStorage (session stable)
      try {
      ...
      const fromUrl = params.get('session') || params.get('sessionId') || params.get('chatId');
      if (fromUrl) return fromUrl;
      // 3. Via API bridge
      if (window.claraverseSyncAPI?.getCurrentSessionId) {
        return window.claraverseSyncAPI.getCurrentSessionId();
      }
      // 4. Générer et stocker
      const newId = `conso_session_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
      try {
        sessionStorage.setItem('claraverse_stable_session', newId);
      } catch (e) {}
      return newId;
    }
    ...
    processAllTables() {
      ...
      tables.forEach((table, index) => {
        if (!this.processedTables.has(table) && !table.classList.contains("claraverse-conso-table")) {
    ...
    processTable(table) {
      try {
        ...
        if (this.isModelizedTable(headers)) {
          ...
          // 🎯 Marquer la position de la table Modelised (3=Modelised)
          table.dataset.tablePosition = '3';
          
          // 🎯 Timestamp de création pour protection contre cleanup prématuré
    ...
    getTableHeaders(table) {
      ...
      for (const selector of headerSelectors) {
        ...
        if (headers.length > 0) {
          // Filtrer pour ne garder que les en-têtes qui appartiennent directement à cette table
          // et non à une table imbriquée
          const directHeaders = Array.from(headers).filter(
            (h) => h.closest("table") === table
          );
          
          if (directHeaders.length > 0) {
            return directHeaders.map((cell, index) => ({
              element: cell,
    ...
    createConsolidationTable(table) {
      ...
      // Phase 3 - Isolation par session pour éviter la contamination inter-chats
      consoTable.dataset.consoSessionId = this.getCurrentSessionId();
      consoTable.dataset.sourceTableId = tableId;
      
      // 🎯 Position dans l'ordre des tables (1=Conso, 2=Résultat, 3=Modelised)
    ...
    insertConsoTable(table, consoTable) {
      ...
      // 🎯 ORDRE CORRECT : Table_conso -> Table_Résultat -> Modelised_table
      // Stratégie : Insérer Table_conso juste avant la Modelised_table (position la plus haute)
      
      // Par défaut : insérer juste avant la Modelised_table (table passée en paramètre)
      let insertBeforeElement = table;
      
      debug.log("✅ Table_conso insérée juste avant Modelised_table");
      
      if (insertBeforeElement && insertBeforeElement.parentElement === parent) {
    ...
    performConsolidation(table) {
      try {
        ...
        this.updateConsolidationDisplay(table, result, consolidationData);
    ...
    updateConsolidationDisplay(table, content, consolidationData) {
      try {
    ...
    updateResultatTable(table, fullContent, consolidationData) {
      debug.log(
      ...
      let resultatTableFound = null; // Track the table we find and update
      ...
      const applyResultatToTable = (potentialTable) => {
        ...
        if (colResultat !== -1) {
            const colAssertion = tableHeaders.findIndex(h => this.matchesColumn(h.text, "assertion"));
            
            // Si la table a une colonne Assertion ET une colonne Resultat, on met à jour ligne par ligne
            if (colAssertion !== -1 && consolidationData && consolidationData.assertions) {
                const tbody = potentialTable.querySelector("tbody") || potentialTable;
                const rows = tbody.querySelectorAll("tr");
                ...
                rows.forEach((row, index) => {
                    ...
                    const assertionCell = cells[colAssertion];
                    const resultatCell = cells[colResultat];
                    
                    if (assertionCell && resultatCell) {
                        const rowAssertion = assertionCell.textContent.trim();
                        if (rowAssertion) {
                            const matchingKey = Object.keys(consolidationData.assertions).find(k => k.toLowerCase() === rowAssertion.toLowerCase());
                            
                            let cellContent = "✅ Satisfaisant";
                            let bgColor = "#efe";
                            
                            if (matchingKey) {
                                const data = consolidationData.assertions[matchingKey];
                                if (consolidationData.type === "withAccount") {
                                    const comptes = Array.from(data.comptes).sort().join(", ");
                                    cellContent = this.generateAssertionPhrase(rowAssertion, comptes, data.total);
                                } else {
                                    cellContent = this.generateSimpleAssertionPhrase(rowAssertion, data.total);
                                }
                                bgColor = "#fee";
                            }
                            ...
                            resultatCell.setAttribute("data-updated", "resultat");
                            updatedAnyRow = true;
                        }
                    }
                });
                ...
                if (updatedAnyRow) {
                    debug.log("✓ Table Résultat structurée mise à jour ligne par ligne");
                    potentialTable.style.outline = "3px solid #00ff00";
                    setTimeout(() => potentialTable.style.outline = "", 2000);
                    
                    // 🎯 Marquer la position de la table (2=Résultat)
                    potentialTable.dataset.tablePosition = '2';
                    
                    // 🎯 Timestamp de création pour protection contre cleanup prématuré
                    potentialTable.dataset.createdTimestamp = Date.now().toString();
                    
                    // 🎯 REPOSITIONNEMENT immédiat après mise à jour réussie
                    this.repositionResultatTable(potentialTable, table);
                    return true;
                }
            }
        }
        
        // Fallback: logique originale de cellule unique
        ...
        if (contentCell) {
            debug.log("✓ Table Résultat trouvée (Fallback cellule unique)");
          ...
          if (sibling.tagName === "TABLE" && sibling !== consoTable) {
            ...
            for (const header of headers) {
              ...
              if (
                ...
                debug.log("✓ Table Résultat trouvée au-dessus de la table conso (S1)");
                if (!consoTable.contains(sibling)) {
                    if (applyResultatToTable(sibling)) return true;
                } else {
                    debug.warn("⚠️ Cette table est la table conso, ignorée");
                }
                break;
              }
            }
      ...
      for (const potentialTable of allTables) {
        ...
        for (const header of headers) {
          ...
          if (
            ...
            if (applyResultatToTable(potentialTable)) return true;
            break;
          }
        }
      }
      ...
      if (resultHeading) {
        ...
        if (foundTable && foundTable !== table && foundTable !== consoTable) {
          ...
          if (applyResultatToTable(foundTable)) return true;
        }
      }
      ...
      for (const t of finalTryTables) {
        ...
        if (text.includes("résultats des tests") || text.includes("résultat des tests")) {
          ...
          if (applyResultatToTable(t)) return true;
        }
      }
      ...
      if (consoTable) {
        ...
        for (let i = 0; i < 5 && prev; i++) {
          ...
          if (text.includes("résultat") || text.includes("resultat")) {
            ...
            // Si c'est une table, utiliser le helper
            if (prev.tagName === "TABLE") {
               if (applyResultatToTable(prev)) return true;
            } else {
               ...
               if (internalTable) {
                  if (applyResultatToTable(internalTable)) return true;
               }
               ...
               if (dataContainer && dataContainer !== consoTable) {
                  if (dataContainer.tagName === "TABLE") {
                     if (applyResultatToTable(dataContainer)) return true;
                  } else {
    ...
    /**
     * Repositionne la table Résultat pour garantir l'ordre: Conso -> Résultat -> Modelised
     * @param {HTMLElement} resultatTable - La table Résultat trouvée
     * @param {HTMLElement} modelisedTable - La table Modelised (table principale)
     */
    repositionResultatTable(resultatTable, modelisedTable) {
      try {
        // Trouver la table Conso
        const tableId = this.generateUniqueTableId(modelisedTable);
        const consoTable = document.querySelector(`table.claraverse-conso-table[data-for-table="${tableId}"]`);
        
        if (!consoTable) {
          debug.warn("⚠️ Table Conso introuvable, impossible de repositionner la table Résultat");
          return;
        }
        
        const parent = modelisedTable.parentElement;
        if (!parent) {
          debug.warn("⚠️ Parent introuvable, impossible de repositionner");
          return;
        }
        
        // Vérifier si la table Résultat est déjà au bon endroit
        // Ordre attendu: Conso, puis Résultat, puis Modelised
        const siblings = Array.from(parent.children);
        const consoIndex = siblings.indexOf(consoTable);
        const resultatIndex = siblings.indexOf(resultatTable);
        const modelisedIndex = siblings.indexOf(modelisedTable);
        
        // Si l'ordre est déjà correct (Conso < Résultat < Modelised), ne rien faire
        if (consoIndex < resultatIndex && resultatIndex < modelisedIndex) {
          debug.log("✅ Table Résultat déjà correctement positionnée");
          return;
        }
        
        // Repositionner: insérer la table Résultat entre Conso et Modelised
        // Stratégie: Insérer juste avant Modelised
        parent.insertBefore(resultatTable, modelisedTable);
        
        debug.log("✅ Table Résultat repositionnée: Conso -> Résultat -> Modelised");
        
        // Effet visuel pour confirmer le repositionnement
        resultatTable.style.outline = "3px solid #ff9800";
        setTimeout(() => resultatTable.style.outline = "", 1500);
        
      } catch (error) {
        debug.error("❌ Erreur lors du repositionnement de la table Résultat:", error);
      }
    }
    ...
    loadAllData() {
      try {
        const sessionKey = `${this.storageKey}_${this.getCurrentSessionId()}`;
    ...
    saveAllData(data) {
      try {
        ...
        debug.log(`💾 Données sauvegardées dans localStorage (session: ${sessionKey})`);
      } catch (error) {
    ...
    saveTableDataNow(table) {
      ...
      // Éviter de sauvegarder les tables vides / en attente de consolidation
      const tableText = table.textContent.trim();
      if (tableText.includes("⏳ En attente de consolidation...") || tableText.includes("En attente de consolidation")) {
        debug.log("⏭️ saveTableDataNow: table vide ou en attente ignorée");
        return;
      }

      const tds = table.querySelectorAll("td");
      if (tds.length > 0) {
        const allEmpty = Array.from(tds).every(td => {
          const txt = td.textContent.trim();
          return txt === "" || txt.includes("⏳ En attente") || txt.includes("En attente de consolidation");
        });
        if (allEmpty) {
          debug.log("⏭️ saveTableDataNow: table ne contient pas de données significatives, ignorée");
          return;
        }
      }
      ...
      // 1. Sauvegarder dans localStorage (fallback)
      this.saveAllData(allData);
      debug.log(`✅ Table ${tableId} sauvegardée dans localStorage`);

      // 2. Sauvegarder dans IndexedDB via le pont si disponible
      if (window.claraverseSyncAPI && window.claraverseSyncAPI.forceSaveTable) {
        window.claraverseSyncAPI.forceSaveTable(table);
        debug.log(`✅ Table ${tableId} envoyée à IndexedDB via pont`);
      } else {
        // Fallback direct via CustomEvent si l'API n'est pas encore prête
        const sessionId = this.getCurrentSessionId();
        const headers = this.getTableHeaders(table);
        const keyword = headers.length > 0 ? headers[0].text.substring(0, 50) : "Table";
        ...
        debug.log(`✅ Table ${tableId} envoyée à IndexedDB via événement direct`);
      }

      debug.log(
        `📊 Total de tables sauvegardées dans localStorage: ${Object.keys(allData).length}`,
      );
    }
