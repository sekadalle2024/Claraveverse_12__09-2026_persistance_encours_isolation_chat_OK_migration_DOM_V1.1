...
import { sessionIsolation } from './sessionIsolationService';
import type { 
...
export class FlowiseTableBridge {
   ...
   * UPDATED: Now uses sessionIsolationService as primary source
   */
  private detectCurrentSession(): void {
    // PRIORITY 1: Use sessionIsolationService (already handles full hierarchy)
    try {
      const sessionId = sessionIsolation.getSessionId();
      if (sessionId) {
        this.currentSessionId = sessionId;
        const diagnostics = sessionIsolation.getDiagnostics();
        console.log(`✅ Session detected from sessionIsolationService: ${sessionId}`);
        console.log(`   Source: ${diagnostics.source}, API Bridge: ${diagnostics.apiBridgeAvailable}`);
        this.logSessionDetection('SessionIsolationService', sessionId, true);
        return;
      }
    } catch (error) {
      console.warn('⚠️ Session detection failed from sessionIsolationService:', error);
      this.logSessionDetection('SessionIsolationService', null, false, error);
    }

    // FALLBACK: Use legacy detection methods (kept for backward compatibility)
   ...
   */
  /**
   * Nettoie toutes les tables générées du DOM avant restauration
   * Prévient l'écrasement et le mélange de données entre sessions
   */
  private async clearAllTablesFromDOM(sessionId?: string): Promise<number> {
    try {
      console.log(`🧹 Nettoyage des tables existantes avant restauration${sessionId ? ` (session: ${sessionId})` : ''}...`);
      
      // Sélectionner toutes les tables générées par Clara
      const selectors = [
        '.clara-generated-table-wrapper',
        '[data-clara-table="true"]',
        '[data-restored-content="true"]',
        '.flowise-table-wrapper'
      ];
      ...
      for (const selector of selectors) {
        ...
        tables.forEach(table => {
          try {
            // Si un sessionId est fourni, vérifier qu'on supprime uniquement les tables de cette session
            if (sessionId) {
              const tableSessionId = table.getAttribute('data-session-id');
              if (tableSessionId && tableSessionId !== sessionId) {
                // Ne pas supprimer les tables d'une autre session
                return;
              }
            }
            ...
            console.warn(`⚠️ Erreur lors de la suppression d'une table:`, error);
          }
        });
      }
      
      console.log(`✅ ${totalCleared} table(s) supprimée(s) du DOM`);
  ...
  public async restoreTablesForSession(sessionId: string): Promise<void> {
    try {
      ...
      // ✅ CRITIQUE: Nettoyer TOUTES les tables de cette session AVANT de restaurer
      // Cela empêche le mélange de données entre différents chats
      const clearedCount = await this.clearAllTablesFromDOM(sessionId);
      console.log(`🧹 Nettoyage terminé: ${clearedCount} table(s) supprimée(s)`);
      ...
      if (successCount > 0) {
        ...
        // ✅ VÉRIFICATION CRITIQUE: Log des tables restaurées pour diagnostic
        const restoredTablesInDOM = document.querySelectorAll(`[data-session-id="${sessionId}"]`);
        console.log(`🔍 VÉRIFICATION: ${restoredTablesInDOM.length} table(s) avec data-session-id="${sessionId}" dans le DOM`);
   ...
   * 🎯 CORRECTION : Protection temporelle (< 5s = nouvelles tables à protéger)
   */
    ...
    try {
      ...
      restoredWrappers.forEach(wrapper => {
        try {
          ...
          console.warn('Error reading restored table wrapper properties:', error);
        }
      });
      
      console.log(`Found ${restoredHeaderSignatures.size} unique restored signature(s) and ${restoredKeywords.size} keyword(s)`);
      
      // Find and remove non-restored tables with matching headers or keywords
      ...
      allTables.forEach(table => {
        ...
        // 1. Check if this table has the same headers as a restored table
        try {
          const headers = Array.from(table.querySelectorAll('th'))
            .map(th => th.textContent?.trim() || '')
            .filter(h => h.length > 0)
          ...
          if (headers && restoredHeaderSignatures.has(headers)) {
            isDuplicate = true;
            matchReason = 'header signature';
          }
        } catch (error) {
          console.warn('Error checking headers for signature match:', error);
        }
        
        // 2. Check if this table has a matching keyword (covers column insert/delete cases)
        if (!isDuplicate) {
          try {
            const keywordAttr = table.closest('[data-n8n-keyword]')?.getAttribute('data-n8n-keyword');
            if (keywordAttr && restoredKeywords.has(keywordAttr.toLowerCase().trim())) {
              isDuplicate = true;
              matchReason = 'keyword attribute';
            } else {
              // Try matching first header or any header containing a restored keyword
              const headers = Array.from(table.querySelectorAll('th'));
              for (const th of headers) {
                const text = th.textContent?.trim().toLowerCase();
                if (text && restoredKeywords.has(text)) {
                  isDuplicate = true;
                  matchReason = `header text "${text}"`;
                  break;
                }
              }
            }
          } catch (error) {
            console.warn('Error checking keywords/headers for duplicate match:', error);
          }
        }
        
        // Remove the table's container if a duplicate is found
        if (isDuplicate) {
          console.log(`🗑️ Removing duplicate original table matched by ${matchReason}`);
  ...
  private createTableWrapper(tableData: FlowiseGeneratedTableRecord): HTMLElement {
    ...
    wrapper.setAttribute('data-session-id', tableData.sessionId); // ✅ AJOUT CRITIQUE: identifier la session
    
    // 🎯 Set position attribute for ordering (position from IndexedDB)
