...
export class FlowiseTableService {
  ...
  async saveGeneratedTable(
    ...
    try {
      // ✅ LOG CRITIQUE: Tracer toutes les sauvegardes
      console.log(`💾 SAUVEGARDE TABLE: sessionId="${sessionId}", keyword="${keyword}", source="${source}", forceUpdate=${forceUpdate}`);
  ...
  private detectTablePosition(tableElement: HTMLTableElement, container: HTMLElement | null): number {
    ...
    // 🎯 First check if table has explicit data-table-position attribute
    ...
    // Fallback: detect position by DOM order
